"""Template writer del importer xlsx (post sections-widgets-redesign).

`build_template()` genera el workbook vacío con las 11 hojas, headers,
...
def build_template() -> BytesIO:
    """Retorna un BytesIO con el template vacío (11 hojas + dropdowns)."""
    wb = build_skeleton()
    return _to_bytes(wb)


def build_skeleton() -> Workbook:
    """Workbook con las 11 hojas, headers y dropdowns — sin filas de data.

    Expuesto para que `excel_exporter` reuse la estructura en vez de
    recrearla. El caller llena las filas y lo exporta con `to_bytes`.
    """
    wb = Workbook()
    ...
    # Sections sheet + all widget sheets
    for sheet_name in s.SHEETS_IN_ORDER:
        if sheet_name in (s.SHEET_INSTRUCCIONES, s.SHEET_REPORTE):
            continue
...
_INSTRUCCIONES_SECTIONS = [
    {
        ...
        "lines": [
            "· Cada reporte tiene un único archivo xlsx con 11 hojas (esta + Reporte + Sections + 8 hojas de widgets).",
            "· La hoja Reporte tiene la data escalar del reporte: tipo, fechas, título, intro, conclusiones.",
            "· La hoja Sections define las secciones del reporte. Cada sección tiene un 'nombre' único en el archivo (ej. 'analisis', 'kpis_mes'). Máximo 60 caracteres, minúsculas, números, guion y guion bajo.",
            "· Las 8 hojas de widgets (Texts, Images, TextImages, KpiGrids, Tables, Charts, TopContents, TopCreators) referencian su sección con 'section_nombre' y su posición dentro de la sección con 'widget_orden' (entero ≥ 1).",
            "· Cada widget se identifica por (section_nombre, widget_orden). Ese par debe ser único por hoja.",
            "· Hojas con sub-items (KpiGrids, Tables, Charts, TopContents, TopCreators): los campos del widget (widget_title, widget_network, etc.) se repiten en cada row del item. Agrupamos por (section_nombre, widget_orden).",
            "· Hoja Tables: tabla genérica con cell_1..cell_8 (variables, dejá vacíos los que no usás). is_header=TRUE marca filas de encabezado. widget_show_total=TRUE agrega una fila Total al final con la suma de las columnas numéricas.",
            "· Hojas sin items (Texts, Images, TextImages): una fila = un widget.",
            "· Hojas vacías significan 'no hay widgets de este tipo en el reporte'. No se rompe nada.",
    ...
    },
    {
        ...
        "lines": [
            ...
            "        ├── intro.jpg",
            "        └── ...",
            "· En las columnas 'imagen' (presentes en Images, TextImages, TopContents, TopCreators) poné SOLO el filename (ej. hero.jpg), sin path.",
    ...
    },
    {
        ...
        "lines": [
            ...
            "",
            "Jerarquía: Report → Section → Widget. Cada Section agrupa widgets temáticamente.",
            "",
            "Tipos de widget disponibles:",
            "· TextWidget — texto libre (body).",
            "· ImageWidget — imagen sola con alt y caption.",
            "· TextImageWidget — texto + imagen con layout interno.",
            "· KpiGridWidget — grilla de tiles con label, value, unit, period_comparison.",
            "· TableWidget — tabla genérica con cell_1..cell_8.",
            "· ChartWidget — gráfico bar/line con puntos (label, value).",
            "· TopContentsWidget — posts/contenidos destacados con métricas.",
            "· TopCreatorsWidget — creadores destacados con métricas.",
            "",
            "Constraints globales:",
            "· 'nombre' de section único en todo el archivo (no repetir entre secciones).",
            "· (section_nombre, widget_orden) único por hoja de widgets.",
            "· En hojas denormalizadas, los widget_* fields deben ser idénticos en todos los rows del mismo widget.",
            "· 'imagen' filenames case-sensitive, extensiones .jpg|.jpeg|.png|.webp, deben existir en images/ del ZIP.",
            "· Enums: 'tipo' ∈ {Influencer, General, Quincenal, Mensual, Cierre de etapa}. 'widget_network' ∈ {Instagram, TikTok, X, (vacío)}. 'source_type' ∈ {Orgánico, Influencer, Pauta, (vacío)}. 'image_position' ∈ {left, right, top}. 'chart_type' ∈ {bar, line}. 'widget_show_total' ∈ {TRUE, FALSE}. 'is_header' ∈ {TRUE, FALSE}. 'layout' ∈ {stack, columns_2, columns_3}.",
            "",
            "Ejemplo canónico (few-shot):",
            "· python manage.py dump_report_example (sin args) escupe un xlsx con el reporte del seed — cubre los 8 widget types. Es la fuente de verdad ante dudas de formato.",
            "",
            ...
            "· No se espera que el LLM extraiga imágenes del PDF. Para campos 'imagen' opcionales, dejarlos vacíos si no están disponibles.",
            "· Para ImageWidget (donde 'imagen' es obligatoria), el LLM debe listar qué filenames referenció para que el usuario provea las imágenes.",
        ],
    },
]
