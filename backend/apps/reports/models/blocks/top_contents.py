"""TopContentsBlock + TopContentItem — posts/contenidos destacados (DEV-129)."""
...
class TopContentsBlock(ReportBlock):
    title = models.CharField(max_length=200, blank=True, default="Top contenidos")
...
class TopContentItem(models.Model):
    block = models.ForeignKey(
        TopContentsBlock, on_delete=models.CASCADE, related_name="items",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["block", "order"],
                name="uniq_item_order_per_top_contents",
            ),
        ]

    def __str__(self):
        return f"{self.block_id} · contenido #{self.order}"
